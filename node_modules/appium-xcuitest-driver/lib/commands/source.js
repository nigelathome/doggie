...
commands.getPageQuickSource = async function () {
  if (this.isWebContext()) {
    const script = 'return document.documentElement.outerHTML';
    return await this.executeAtom('execute_script', [script, []]);
  }
  if (this.settings.getSettings().useJSONSource) {
    ...
    return await this.getPageQuickSource();
  }
};
helpers.getPageQuickSource = async function () {
  let srcTree = await this.proxyCommand('/quickPageSource', 'GET');
  return srcTree;
};
